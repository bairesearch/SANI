				...
				if(componentTests(ownerComponent, ownerGroup, forwardPropogationSignalData, forwardPropogationWordData, forwardPropogationSentenceData))
				{
					...
					for(int i2=0; i2<ownerGroup->currentParseTreeGroupArray.size(); i2++)
					{
						...
						if(isLastComponentFuzzy(ownerGroup))
						{
							...
							{
								GIAtxtRelTranslatorRulesComponent* currentParseTreeComponent = NULL;
								if(findComponentRef(activationPathWordCurrentParseTreeGroupOwner, lastComponent, &currentParseTreeComponent))
								{
									if(currentParseTreeComponent->neuronComponentConnectionActive)
									{
										if(lastComponent->componentType == GIA_TXT_REL_TRANSLATOR_RULES_GROUPS_COMPONENT_COMPONENTTYPE_REPEAT)
										{
											if(currentParseTreeComponent->neuronComponentConnectionActiveWordRecord->translatorSentenceEntityIndex == forwardPropogationWordData->w - 1)
											{
									...
									{
										cerr << "GIAtxtRelTranslatorNeuralNetworkClass::propagateWordThroughNetworkGroupSelect error{}: findComponentRef && !(currentParseTreeComponent->neuronComponentConnectionActive)" << endl;
										exit(EXIT_ERROR);
									}
...
bool GIAtxtRelTranslatorNeuralNetworkClass::propagateWordThroughNetworkGroupComponentWrapper(GIAtranslatorVariablesClass* translatorVariables, GIAtxtRelTranslatorRulesGroup* group, GIAtxtRelTranslatorRulesComponent* currentComponent, GIAtxtRelTranslatorRulesGroup* ownerGroup, GIAtxtRelTranslatorRulesComponent* ownerComponent, GIAtxtRelTranslatorNeuralNetworkForwardPropogationSignalData* forwardPropogationSignalData, GIAtxtRelTranslatorNeuralNetworkForwardPropogationWordData* forwardPropogationWordData, GIAtxtRelTranslatorNeuralNetworkForwardPropogationSentenceData* forwardPropogationSentenceData, int layer, GIAtxtRelTranslatorRulesGroup* activationPathWordCurrentParseTreeGroup, GIAtxtRelTranslatorRulesGroup* activationPathWordCurrentParseTreeGroupOwner, const bool lastComponentFuzzyAndCompatible)
{
	...
	if(propagateWordThroughNetworkGroupVerifyComponentSequenceActivationReady(ownerComponent, &(ownerGroup->components), activationPathWordCurrentParseTreeGroupOwner, forwardPropogationSignalData, forwardPropogationWordData, forwardPropogationSentenceData, &activationSequenceCompleted))
	{
		/*
		#ifdef GIA_DEBUG_TXT_REL_TRANSLATOR_NEURAL_NETWORK_PROPAGATE
		//GIAtxtRelTranslatorRules.printParseTreeDebugIndentation(layer+1);
		...
		*/
		...
		if(subComponentsPassed)
		{
			...
			if(propagateWordThroughNetworkGroupComponent(translatorVariables, group, currentComponent, ownerGroup, ownerComponent, forwardPropogationSignalData, forwardPropogationWordData, forwardPropogationSentenceData, activationSequenceCompleted, layer, repeatDetected, activationPathWordCurrentParseTreeGroup, activationPathWordCurrentParseTreeGroupOwner, lastComponentFuzzyAndCompatible))
			{
				result = true;
			}
		}
...
bool GIAtxtRelTranslatorNeuralNetworkClass::propagateWordThroughNetworkGroupComponent(GIAtranslatorVariablesClass* translatorVariables, GIAtxtRelTranslatorRulesGroup* group, GIAtxtRelTranslatorRulesComponent* currentComponent, GIAtxtRelTranslatorRulesGroup* ownerGroup, GIAtxtRelTranslatorRulesComponent* ownerComponent, GIAtxtRelTranslatorNeuralNetworkForwardPropogationSignalData* forwardPropogationSignalData, GIAtxtRelTranslatorNeuralNetworkForwardPropogationWordData* forwardPropogationWordData, GIAtxtRelTranslatorNeuralNetworkForwardPropogationSentenceData* forwardPropogationSentenceData, const bool activationSequenceCompleted, int layer, const bool repeatDetected, GIAtxtRelTranslatorRulesGroup* activationPathWordCurrentParseTreeGroup, GIAtxtRelTranslatorRulesGroup* activationPathWordCurrentParseTreeGroupOwner, const bool lastComponentFuzzyAndCompatible)
{
	bool result = false;
	...
	else
	{
		if(activationSequenceCompleted)
		{
			...
			activationPathWordCurrentParseTreeGroupOwner->neuronActive = true;
...
bool GIAtxtRelTranslatorNeuralNetworkClass::deleteComponents(GIAtxtRelTranslatorRulesGroup* currentParseTreeGroup)
{
	bool result = true;
...
{	
	...
	for(int i=0; i<components->size(); i++)
	{
		...
		{
			/*
			cout << "currentParseTreeComponent->neuronComponentConnectionActive = " << currentParseTreeComponent->neuronComponentConnectionActive << endl;
			...
			*/
			if(component == testComponent)
			{
				...
				if(findComponentRef(activationPathWordCurrentParseTreeGroupOwner, component, &currentParseTreeComponent))
				{
					if(currentParseTreeComponent->neuronComponentConnectionActive)
					{
					...
					{
						//sequential activation found
						cerr << "GIAtxtRelTranslatorNeuralNetworkClass::propagateWordThroughNetworkGroupVerifyComponentSequenceActivationReady error{}: findComponentRef && !(currentParseTreeComponent->neuronComponentConnectionActive)" << endl;
						exit(EXIT_ERROR);
						//cout << "activate" << endl;
					}
				}
			...
			else
			{
				...
				if(findComponentRef(activationPathWordCurrentParseTreeGroupOwner, component, &currentParseTreeComponent))
				{
					...
					if(currentParseTreeComponent->neuronComponentConnectionActive)
					{
						...
						previousActiveComponent = currentParseTreeComponent;
					}
					#endif
	...
	for(int i=0; i<components->size(); i++)
	{
		...
		if(component == testComponent)
		{
			if(findComponentRef(activationPathWordCurrentParseTreeGroupOwner, component, &currentParseTreeComponent))
			{
				...
				{
					cerr << "GIAtxtRelTranslatorNeuralNetworkClass::propagateWordThroughNetworkGroupVerifyComponentSequenceOrActivationReady error{}: findComponentRef && !(currentParseTreeComponent->neuronComponentConnectionActive)" << endl;
					exit(EXIT_ERROR);
				}
			}
			else
		...
		if(component == testComponent)
		{
			...
			if(testComponent->componentType == GIA_TXT_REL_TRANSLATOR_RULES_GROUPS_COMPONENT_COMPONENTTYPE_STRING)
			{
				if(findComponentRef(activationPathWordCurrentParseTreeGroupOwner, component, &currentParseTreeComponent))
				{
					...
					{
						pass = false;
						if(currentParseTreeComponent->neuronComponentConnectionActiveWordRecord->translatorSentenceEntityIndex == forwardPropogationWordData->wordReference->translatorSentenceEntityIndex - 1)
						{
							pass = true;
						}
					}
					...
					{
						cerr << "GIAtxtRelTranslatorNeuralNetworkClass::propagateWordThroughNetworkGroupVerifyComponentSequenceRepeatActivationReady error{}: findComponentRef && !(currentParseTreeComponent->neuronComponentConnectionActive)" << endl;
						exit(EXIT_ERROR);
					}
				}
			}
...
{	
	...
	/*
	group->currentParseTreeGroupArray = new GIAtxtRelTranslatorRulesGroup(*group);
	group->currentParseTreeGroupArray->components.clear();	//added 3g1h
	*/
